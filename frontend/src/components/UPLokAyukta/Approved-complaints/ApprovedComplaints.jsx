...
const ForwardModal = ({ 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post(`/uplokayukt/forward-by-so/${complaintId}`, {
...
const ApprovedComplaints = () => {
  ...
  const fetchApprovedComplaints = async () => {
    ...
    try {
      const response = await api.get("/uplokayukt/all-approved-complaints");
  ...
  const handleViewDetails = (e, complaintId) => {
    e.stopPropagation();
    navigate(`/uplokayukt/approved-complaints/view/${complaintId}`);
