...
const PreView = ({ complaintData }) => {
  ...
  const data = useMemo(() => {
    ...
    const decode = (txt) => krutiToUnicode(txt || "");

    return {
      ...raw,
      complain_no: decode(raw.complain_no),
      complaint_description: decode(raw.complaint_description),
      delay_reason: decode(raw.delay_reason),
      category: raw.category,
      attached_documents_description: decode(raw.attached_documents_description),
      previously_submitted_details: decode(raw.previously_submitted_details),
      challan_number: decode(raw.challan_number),

      // Correspondence Address
      correspondence_name: decode(raw.correspondence_name),
      correspondence_place: decode(raw.correspondence_place),
      correspondence_post_office: decode(raw.correspondence_post_office),
      correspondence_district: raw.correspondence_district,

      complainants: raw.complainants?.map((comp) => ({
        ...comp,
        ...
        permanent_post_office: decode(comp.permanent_post_office),
      })) || [],

      respondant: raw.respondant?.map((resp) => ({
        ...resp,
        respondent_name: decode(resp.respondent_name),
        designation: resp.designation,
        department_name: resp.department_name,
        district_name: resp.district_name || resp.respondent_district,
        current_address: decode(resp.current_address),
        officer_category: decode(resp.officer_category),
      })) || [],

      support: raw.support?.map((item) => ({
        ...item,
        support_name: decode(item.support_name),
        support_address: decode(item.support_address),
      })) || [],

      witness: raw.witness?.map((item) => ({
        ...item,
  ...
  return (
    <div className="w-full h-full bg-[#FFFBF2] rounded-lg flex flex-col overflow-hidden shadow-xl border ">
      ...
      <div className="flex-1 overflow-y-auto bg-[#FFFBF2] p-6 md:p-8 flex justify-center">
        <div
          ref={contentRef}
          className="w-full max-w-4xl text-[13px] text-gray-900 leading-relaxed bg-[#FFFBF2] p-8"
        >
          <div className="text-center mb-8">
            <h1 className="font-semibold text-sm">
              "शिकायत", "अभिकथन" उत्तर प्रदेश लोक आयुक्त तथा माननीय उप लोक आयुक्त
              अधिनियम, 1975
            </h1>
          </div>
          ...
          <div className="space-y-4">
              ...
              <div className="flex-1">
                ...
                {validComplainants.length > 0 ? (
                  validComplainants.map((comp, index) => (
                    <div
                      key={index}
                      className="pl-4 mt-1 font-medium  border-gray-300 pb-1 last:border-0"
                    >
                      <span className="font-bold text-gray-700">
                        परिवादी {index + 1}:
                      </span>{" "}
                      {safeData(comp.complainant_name)}
                    </div>
                  ))
                ) : (
                  ...
                  {validComplainants.length > 0 ? (
                    validComplainants.map((comp, index) => (
                      ...
                      >
                        ...
                        {safeData(comp.is_public_servant)}
                      </div>
                    ))
                  ) : (
                    <div className="pl-4 mt-0.5 font-medium">NA</div>
              ...
              <div className="flex-1">
                ...
                {validComplainants.length > 0 ? (
                  validComplainants.map((comp, index) => (
                    <div
                      key={index}
                      className="pl-4 mt-2 font-medium rounded p-2 mb-2 border border-dashed border-gray-400"
                    >
                      <div className="font-bold text-blue-800 underline mb-1">
                        परिवादी {index + 1}
                      </div>
                      <p>
                        ...
                        {safeData(comp.permanent_place)}
                      </p>
                      <p>
                        <span className="font-semibold text-gray-700">
                          (ख) डाकघर :
                        </span>{" "}
                        {safeData(comp.permanent_post_office)}
                      </p>
                      <p>
                        <span className="font-semibold text-gray-700">
                          (ग) जिला :
                        </span>{" "}
                        {safeData(comp.district_name)}
                      </p>
                    </div>
                  ))
                ) : (
            ...
            <div className="flex gap-2">
              ...
              <div className="flex-1">
                ...
                <div className="pl-4 mt-0.5 space-y-0.5 font-medium">
                  ...
                  <p>
                    (ग) डाकघर/थाना : {safeData(data.correspondence_post_office)}
                  </p>
            ...
            <div className="flex gap-2">
              ...
              <div className="flex-1">
                <span>
                  (1) जिस व्यक्ति के विरूद्ध परिवाद किया जा रहा है उसका नाम,
                  पदनाम और पता :
                </span>
                      ...
                      >
                        ...
                        <div className="space-y-1 font-medium text-gray-800 text-sm">
                          <div className="grid grid-cols-[80px_1fr]">
                            ...
                            <span>{safeData(resp.respondent_name)}</span>
                          </div>
                          <div className="grid grid-cols-[80px_1fr]">
                            <span className="font-semibold text-gray-900">
                              पदनाम:
                            </span>{" "}
                            <span>{safeData(resp.designation)}</span>
                          </div>
                          <div className="grid grid-cols-[80px_1fr]">
                            <span className="font-semibold text-gray-900">
                              विभाग:
                            </span>{" "}
                            <span>{safeData(resp.department_name)}</span>
                          </div>
                          <div className="grid grid-cols-[80px_1fr]">
                            <span className="font-semibold text-gray-900">
                              जिला:
                            ...
                            <span>
                              {safeData(
                                resp.district_name || resp.respondent_district
                              )}
                            </span>
                          </div>
                          <div className="grid grid-cols-[80px_1fr]">
                            ...
                            <span>{safeData(resp.current_address)}</span>
                          </div>
                        </div>
                      </div>
                    ))
                ...
                <div className="mt-2">
                  ...
                  <div className="pl-4 mt-0.5 font-medium">
                    {safeData(data.delay_reason, "कोई विलम्ब नहीं")}
                  </div>
                </div>

                <div className="mt-2">
                  (4) क्या परिवाद पहले किसी वरिष्ठ अधिकारी के समक्ष किया गया
                  था?
                  <div className="pl-4 mt-0.5 font-medium">
                    {data.previously_submitted === "yes" ? "हाँ" : "नहीं"}
                    {data.previously_submitted === "yes" &&
                      ` - ${safeData(data.previously_submitted_details)}`}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <div className="flex-1">
                <span>
                  परिवाद का विवरण - कृपया यहाँ पर परिवाद के सम्पूर्ण तथ्य
                  बतायें।
                </span>
