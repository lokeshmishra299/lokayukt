...
// import { FaSearch, FaDownload, FaFileAlt, FaChartBar, FaSpinner,  FaArrowRight,    FaChevronDown,FaUser,FaUserTie,FaCrown,FaUsers,FaTimes,
// } from "react-icons/fa";
...
//   const selectedOption = flattenOptions(options).find(
//     (opt) => opt.value === value
//   );
...
//       saveAs(
//         data,
//         `Search_Reports_${new Date().toISOString().slice(0, 10)}.xlsx`
//       );
...
//         try {
//           const districtWiseResponse = await api.get(
//             "/operator/district-wise-complaint"
//           );
...
//         try {
//           const departmentWiseResponse = await api.get(
//             "/operator/department-wise-complaint"
//           );
...
//         try {
//           const monthlyTrendsResponse = await api.get(
//             "/operator/montly-trends"
//           );
...
//         try {
//           const avgProcessingResponse = await api.get(
//             "/operator/detail-by-complaintype"
//           );
...
//         try {
//           const complianceReportResponse = await api.get(
//             "/operator/compliance-report"
//           );
...
//     if (status === "Pending") return "bg-green-400 text-white";
//   };
...
//         matchesDistrict =
//           result.district_id.toString() === selectedDistrictObj.district_code;
//       } else {
...
//     ).length,
//     rejected:
//       overallStats?.total_rejected ||
//       ensureArray(searchResults).filter((r) => r.status === "Rejected").length,
//     inProgress:
//       overallStats?.total_pending ||
//       ensureArray(searchResults).filter(
//         (r) =>
...
//     const validTimes = avgProcessingTimes.filter(
//       (item) => item.avg_days !== null && !isNaN(parseFloat(item.avg_days))
//     );
//     if (validTimes.length === 0) return "N/A";

//     const totalDays = validTimes.reduce(
//       (sum, item) => sum + parseFloat(item.avg_days),
//       0
//     );
...
//                                   <td
//                                     colSpan="9"
//                                     className="py-8 text-center font-semibold text-md text-gray-500"
//                                   >
//                                     Loading...
//                                   </td>
//                                 </tr>
...
//                                   >
//                                     <td
//                                       className="py-2 px-2 sm:px-3 font-medium text-gray-700 hover:text-blue-800 hover:underline cursor-pointer"
//                                       onClick={() =>
//                                         navigate(
//                                           `/operator/search-reports/view/${result.id}`
//                                         )
//                                       }
//                                     >
//                                       {result.complain_no ||
...
//                                           <span className="hidden text-green-600 font-semibold sm:inline">
//                                             View
//                                           </span>
//                                         </button>
//                                       </div>
...
//                               {districtWiseStats &&
//                               Object.keys(districtWiseStats).length > 0 ? (
//                                 Object.entries(districtWiseStats).map(
//                                   ([districtName, count]) => (
//                                     <div
//                                       key={districtName}
...
//                             <div className="space-y-3 max-h-80 overflow-y-auto">
//                               {departmentWiseStats &&
//                               Object.keys(departmentWiseStats).length > 0 ? (
//                                 Object.entries(departmentWiseStats).map(
...
//                                   {parseFloat(
//                                     complianceReport.approved_percentage
//                                   ).toFixed(1)}
//                                   %
//                                 </div>
...
//                                   {parseFloat(
//                                     complianceReport.pending_percentage
//                                   ).toFixed(1)}
//                                   %
//                                 </div>
//                                 <div className="text-xs sm:text-sm text-gray-500">
...
//                                   {parseFloat(
//                                     complianceReport.rejected_percentage
//                                   ).toFixed(1)}
//                                   %
//                                 </div>
//                                 <div className="text-xs sm:text-sm text-gray-500">
...
export default function InboxUI() {
  return (
    <div className="w-full h-screen flex bg-gray-100 px-4 py-3 gap-4">

      {/* LEFT PANEL */}
      <div className="w-[42%] bg-white rounded-xl border shadow-sm p-4 flex flex-col overflow-hidden">

        {/* HEADER ROW */}
        <div className="flex items-center gap-20 mb-3 text-[10px]">
          <div className="">
          <h2 className="text-[18px] font-semibold">Inbox</h2></div>
        ...
        {/* OVERDUE + FEE PENDING TABS */}
        <div className="flex gap-3 mb-3 text-[10px]">
          <button className="px-4 py-1 bg-yellow-50 border border-yellow-200 rounded-[8px] text-yellow-700">
            Overdue 7 days (5)
          </button>

          <button className="px-4 py-1 bg-purple-50 border border-purple-200 rounded-[8px] text-purple-700">
            Fee Pending (8)
          </button>
        </div>
  ...
  <input
    type="text"
    className="w-full border rounded-lg pl-10 pr-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500"
    placeholder="Search by file no., complainant, subject..."
  />
</div>
        {/* FILTERS */}
        <div className=" w-[80%] grid grid-cols-3 gap-3 mb-3 text-[11px]">
          <select className="border w-24 p-2  rounded-lg">
        ...
        <div className=" w-full flex justify-between  mb-3 text-[11px]">
          <select className="border p-2 rounded-lg">
          ...
          {/* SORT */}
        {/* <div className="mb-3"> */}
          <select className="border w-40 p-2 rounded-lg ">
 ...
 <div className="h-full  bg-gray-50 px-1 py-3">

  {/* FILE LIST (SCROLLABLE) */}
  <div className="flex overflow-y-auto pr-2 flex-1 space-y-4 customScrollbar flex-col">

   {/* ==== CASE 1 ==== */}
<div className="grid grid-cols-2 border rounded-xl p-4 bg-white shadow-sm">

  {/* LEFT SIDE SECTION */}
  <div className="flex flex-col w-full">
    <p className="text-sm font-semibold text-gray-700">File No. 2025/LOK/123</p>

    <p className="text-xs text-gray-600 mt-1">
      Complaint regarding irregularities in tender...
    </p>

    <p className="text-xs text-gray-600 mt-2">
      Complainant: <span className="font-medium">Shri Ramesh Kumar</span> • District: Lucknow
    </p>
   
  </div>

  {/* RIGHT SIDE SECTION */}
  <div className="flex flex-col items-center gap-3 w-full text-[12px] px-5">

  {/* TAG GRID */}
  <div className="grid grid-cols-2 gap-2 w-full  text-[10px] ">

    <span className="px-2 py-1 h-6 bg-blue-100 text-blue-700 rounded-md text-center">
      New Case
    </span>

    <span className="px-2 py-1 h-6 w-[100px] bg-purple-100 text-purple-700 rounded-md text-center">
      With UpLokayukta
    </span>

    <span className="px-2 py-1 bg-gray-200 text-gray-700 rounded-md text-center">
      2 days
    </span>

    <span className="px-2 py-1 bg-yellow-100 text-yellow-700 rounded-md text-center">
      Fee Partial
    </span>
  </div>
</div>
    ...
    <div className="border rounded-xl p-3 bg-white shadow-sm">
      <p className="text-sm font-semibold text-gray-700">File No. 2025/LOK/118</p>

      <p className="text-[14px] text-gray-600 mt-1">
        Allegations of corruption in land acquisition process...
      </p>
      {/* TAGS */}
      
      <div className="grid grid-cols-2 gap-2 mt-2 text-[12px]">
        <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-md">New Case</span>
        <span className="px-2 py-1 bg-purple-100 text-purple-700 rounded-md">With UpLokayukta</span>
        <span className="px-2 py-1 bg-red-100 text-red-700 rounded-md">&gt;9 days</span>
        <span className="px-2 py-1 bg-green-100 text-green-700 rounded-md">Fee Paid</span>
        <p className="text-[12px] text-gray-500 mt-1">
        Received: 03 Jan 2025 • Last action: 14 Jan 2025
      </p>
      </div>

      {/* DETAILS */}
      <p className="text-[13px] text-gray-600 mt-2">
        Complainant: <span className="font-medium">Smt. Priya Sharma</span> • District: Noida
      </p>
      ...
      {/* RIGHT PANEL */}
      <div className="w-[58%] bg-white rounded-xl border shadow-sm flex items-center justify-center">
        <div className="text-center text-gray-500">
          <div className="text-6xl mb-3">📄</div>
          <p className="text-[16px]">Select a file to view details</p>
        </div>
      </div>
