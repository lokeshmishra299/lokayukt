...
// ✅ Accept 'closeModal' prop from Parent
const EditDraft = ({ closeModal }) => {
  ...
  const handleSubmitNote = () => {
    ...
    // alert("Draft Submitted! Check console for content."); // Optional
    
    // Close the modal via Parent function
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4 md:p-0">
      <div className="bg-white w-full md:w-11/12 h-full md:h-[70vh] shadow-xl relative flex flex-col md:flex-row rounded-md overflow-hidden animate-fade-in-up">
        
        {/* Close Button calling PARENT function */}
        <button
          className="absolute top-2 right-2 md:top-4 md:right-4 p-2 bg-white/80 hover:bg-gray-100 rounded-full z-10"
          onClick={closeModal} // ✅ Correct: Calls parent to unmount
        >
          <FaTimes className="w-5 h-5 text-gray-600" />
        </button>

        <div className="flex-1 px-4 md:p-6 overflow-y-auto">
          <h2 className="text-lg font-semibold mb-4 md:mb-6">Edit Draft</h2>
          ...
          {/* Submit Button */}
          <div className="flex justify-end mt-4 md:mt-6 pb-4">
            <button
