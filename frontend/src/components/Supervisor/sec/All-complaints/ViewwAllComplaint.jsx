...
//  DSC LOGIC
// NIC DSCSigner (ya NIC Digital Signer Service, emSigner (eMudhra), Signer.Digital (Private) 


// Signer	Port ->	Certificate API
// NIC DSCSigner ->	55101	/getSignerCertificate
// eMudhra emSigner ->	1585	/getCert
// Sify	2095 ->	/listCert
// Capricorn	different	different
// const signWithDSC = async () => {
//   try {
//     const dscPort = 55101; 
//     const dscUrl = `https://127.0.0.1:${dscPort}`;
//     console.log("Connecting...", dscUrl);

//     try {
//       await axios.get(`${dscUrl}/`, { timeout: 2000 });
//     } catch (e) {
//       toast.error("DSC Signer not running! Start NIC DSCSigner.");
//       return null; 
//     }

//     const response = await axios.post(`${dscUrl}/getSignerCertificate`, { "action": "list" }, {
//       headers: { "Content-Type": "application/json" }
//     });

//     if (response.data && response.data.status === "success") {
//        const cert = response.data.certificateData || response.data;
       
//        // UI Update (Optional but good for display)
//        setCertSubject(cert.subject || "");
       
//        // Prepare Data Object
//        const dscData = {
//           signatureData: "SIGNED_" + new Date().getTime() + "_" + (cert.serialNumber || ""),
//           plainText: "00",
//           certThumbprint: cert.thumbprint || cert.Thumbprint || "",
//           certSubject: cert.subject || cert.Subject || "",
//           certSerialNo: cert.serialNumber || cert.SerialNumber || "",
//           validFrom: cert.validFrom || "",
//           validTo: cert.validTo || "",
//           digitalId: cert.subject || cert.Subject || "",
//           cstInfo: cert.issuer || cert.Issuer || ""
//        };

//        toast.success("DSC Connected Successfully! ✅");
//        return dscData; 
//     } else {
//        toast.error("DSC Token not found.");
//        return null;
//     }
//   } catch (err) {
//     console.error(err);
//     toast.error("DSC Connection Failed.");
//     return null;
//   }
// };
...
// ===============================================
// CHANGE 1: New Function for HyperPKI WebSigner
// ===============================================

// const signWithHyperPKI = async () => {
//   try {
//     // HyperPKI Port usually: 20202 (HTTPS) or 20201 (HTTP)
//     const dscUrl = "http://127.0.0.1:20202"; 
    
//     console.log("Connecting to HyperPKI Service...", dscUrl);
...
//     toast.error("Connection Error: Check if Token is plugged in.");
//     return null;
//   }
// };
...
//  2. Update forwardComplaintMutation
const forwardComplaintMutation = useMutation({
  mutationFn: async ({ complaintId, forwardTo, remarkData, dscData }) => { 
    const dsc = dscData || {}; 
    ...
    const res = await api.post(
      `/supervisor/forward-by-supervisor/${complaintId}`,
      {
        ...
        txtsigndata: dsc.signatureData || signatureData,        
        txtPlainText: dsc.plainText || plainText,              
        HF1: dsc.certThumbprint || certThumbprint,                  
        HF3: dsc.certSubject || certSubject,                     
        HF2: dsc.certSerialNo || certSerialNo,                    
        serialno: dsc.certSerialNo || certSerialNo,              
        hdnValidityEnd: dsc.validTo || validTo,             
        hdnValidityfrom: dsc.validFrom || validFrom,          
        digitalID: dsc.digitalId || digitalId,                
        cstinfo: dsc.cstInfo || cstInfo,                    
  ...
  //     if (sent_through_rk) {
  //       // Wait karo signature ka
  //       dscData = await signWithDSC(); 
...
const handleConfirmYes = async () => {
    if (confirmConfig.type === "receive") {
      ...
      if (sent_through_rk) {
        ...
        dscData = await signWithHyperPKI(); 
